...
namespace DAL_QLNT
{
     public class UsersDAL
    {
        ...
        public bool ThemUsers(string taiKhoan,string matKhau,string ten,string quyen )
        {
            ...
            string query = string.Format("INSERT INTO NGUOIDUNG(TAIKHOAN,MATKHAU,TEN,QUYEN) VALUES (N'{0}',N'{1}','{2}','{3}')", taiKhoan, matKhau, ten, quyen );
        ...
        {
            int kq = 0;
            string query = string.Format("UPDATE NGUOIDUNG SET TAIKHOAN = '{0}', MATKHAU = '{1}', TEN = N'{2}', QUYEN = N'{3}' WHERE ID = {4}",taiKhoan, matKhau, ten, quyen, id );
            kq = DataProvider.Instance.ExecuteNonQuery(query);
            return kq > 0;
        }
